...
const UserCredentials = ({ credential, label }) => {

  return (
    <span className="flex justify-between items-center text-[#c4c3c3] font-raleway cursor-pointer">{label}
      <HoverEffectWrapper>
        <h3 className="text-lg font-semibold font-spaceMono text-right text-[#EEE]"
          style={{
...
const UserProfilePage = () => {
  ...
  return (
    <div className="p-4 flex flex-col md:grid gap-2  md:grid-cols-[1fr__2fr] h-full md:h-screen hide-scrollbar overflow-y-scroll">
      <div className="flex flex-col rounded-xl bg-[#222831] overflow-hidden">
        <div className="h-4/5 ">
          ...
          <div className="flex flex-col gap-2 text-[#EEE] p-4 mt-4">
            <UserCredentials credential={user?.name} label={"Name"} />
            <UserCredentials credential={user?.email} label={"Email"} />
          </div>
        </div>
          <div className="h-2/4 flex flex-col justify-end pb-4 px-4">
            <button onClick={handleLogout} className="p-3 rounded-lg text-xl font-semibold 
          font-spaceMono bg-[#D61A3C] text-[#EEE] hover:bg-[#B2022F] transition delay-75">
              Log out
            </button>
          </div>
      </div>
        ...
        <HoverEffectWrapper>
          <div className="w-full flex justify-end items-end">
            <button
              ...
              className=" w-full md:w-auto bg-[#222831] text-[#EEE] hover:bg-[#EEE] hover:text-[#222831] 
            hover:border-[#EEE] border-2 border-[#222831] 
            font-spaceMono font-semibold text-lg rounded-md py-3 px-4 transition delay-150"
            >
              Post a video
            </button>
          </div>
